...
function startTracking(config) {
  const { symbol, exchange1, exchange2 } = config;
  activeConfig = config;
  ...
  intervalId = setInterval(() => {
    Promise.all([
      ...
      .catch(err => {
        console.error('[ERROR] Ошибка при запросе данных:', err);
      });
  }, 500);
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "start-tracking") {
    const { exchange1, exchange2, symbol } = message;
    startTracking({ exchange1, exchange2, symbol });
